...
class ZoneController extends BaseController
{
    ...
    */
    public function index()
    {
        ...
        $main_menu = 'map';
        $sub_menu = 'zone';
$zones = Zone::orderBy('position')->get();
        return view('admin.zone.index', compact('page', 'main_menu', 'sub_menu','zones'));
    }
    ...
    */
    ...
    public function store(CreateZoneRequest $request)
    {
        ...
        foreach (json_decode($request->coordinates) as $key => $coordinates) {
            ...
            foreach ($coordinates as $key => $coordinate) {
                if ($key == 0) {
                    $created_params['latitude'] = $coordinate->lat;
                    $created_params['longitude'] = $coordinate->lng;
                }
        ...
        $created_params['city'] = $request->input('city');

        $created_params['coordinates'] = $multi_polygon;

        $created_params['company_key'] = auth()->user()->company_key;
 $lastPosition = $this->zone->companyKey()->max('position');

        $created_params['position'] = $lastPosition + 1;
    ...
    */
    public function update(Zone $zone, CreateZoneRequest $request)
    {
        ...
        foreach (json_decode($request->coordinates) as $key => $coordinates) {
            ...
            foreach ($coordinates as $key => $coordinate) {
                if ($key == 0) {
                    $updated_params['latitude'] = $coordinate->lat;
                    $updated_params['longitude'] = $coordinate->lng;
                }
                $point = new Point($coordinate->lat, $coordinate->lng);
        ...
        $updated_params['city'] =$request->input('city');

$updated_params['seat_status'] = $request->input('seat_status');
...
{
    $zone1Id = $request->input('zone1_id');
    $zone2Id = $request->input('zone2_id');

    $zone1 = Zone::find($zone1Id);
    $zone2 = Zone::find($zone2Id);

    $position1 = $zone1->position;
    $position2 = $zone2->position;

    // Swap positions
    $zone1->position = $position2;
    $zone2->position = $position1;

    $zone1->save();
    $zone2->save();
    
    return redirect()->back()->with('success', 'Positions swapped successfully.');
}
}
